...
async function scrapeInstagramCarousel(url: string) {
  ...
  try {
    ...
    while (true) {
      const images = await page.$$eval('article img', (imgs: HTMLImageElement[]) =>
        imgs.map((img) => img.src)
      );
      for (const src of images) {
        imageUrls.add(src);
      }
      console.log(`Found ${images.length} images in current view, total unique: ${imageUrls.size}`);
...
if (!instagramUrl) {
  console.error('Please provide an Instagram post URL as a command-line argument.');
  process.exit(1);
}
