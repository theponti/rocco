...
interface SeasonalAnalysis {
	...
	peakMonth: number;
	troughMonth: number;
	deathSeasonalityStrength: number;
	deathPeakMonth: number;
	deathTroughMonth: number;
...
export async function loader({ request }: LoaderFunctionArgs) {
	try {
		...
		for (let month = 1; month <= 12; month++) {
			...
			const averageCases =
				cases.length > 0
					? cases.reduce((sum, val) => sum + val, 0) / cases.length
					: 0;

			const averageDeaths =
				deaths.length > 0
					? deaths.reduce((sum, val) => sum + val, 0) / deaths.length
					: 0;

			// Calculate variance
			const caseVariance =
				cases.length > 1
					? cases.reduce((sum, val) => sum + (val - averageCases) ** 2, 0) /
						(cases.length - 1)
					: 0;

			const deathVariance =
				deaths.length > 1
					? deaths.reduce((sum, val) => sum + (val - averageDeaths) ** 2, 0) /
						(deaths.length - 1)
					: 0;
		...
		const caseAverages = patterns
			.map((p) => p.averageCases)
			.filter((avg) => avg > 0);
		const deathAverages = patterns
			.map((p) => p.averageDeaths)
			.filter((avg) => avg > 0);

		let seasonalityStrength = 0;
		let peakMonth = 1;
		let troughMonth = 1;
		let deathSeasonalityStrength = 0;
		let deathPeakMonth = 1;
		let deathTroughMonth = 1;

		if (caseAverages.length > 6) {
			const maxCases = Math.max(...caseAverages);
			const minCases = Math.min(...caseAverages);
			const meanCases =
				caseAverages.reduce((sum, val) => sum + val, 0) / caseAverages.length;

			// Seasonality strength as coefficient of variation
			seasonalityStrength =
				meanCases > 0 ? (maxCases - minCases) / meanCases : 0;
		...
		// Calculate death seasonality metrics
		if (deathAverages.length > 6) {
			const maxDeaths = Math.max(...deathAverages);
			const minDeaths = Math.min(...deathAverages);
			const meanDeaths =
				deathAverages.reduce((sum, val) => sum + val, 0) / deathAverages.length;

			// Death seasonality strength as coefficient of variation
			deathSeasonalityStrength =
				meanDeaths > 0 ? (maxDeaths - minDeaths) / meanDeaths : 0;

			// Find death peak and trough months
			deathPeakMonth =
				patterns.findIndex((p) => p.averageDeaths === maxDeaths) + 1;
			deathTroughMonth =
				patterns.findIndex((p) => p.averageDeaths === minDeaths) + 1;
		}
		...
		const detectPatterns = (patterns: SeasonalPattern[]) => {
			...
			const winterAvg =
				winterMonths.reduce(
					(sum, month) => sum + patterns[month - 1].averageCases,
					0,
				) / 3;
			const overallAvg =
				patterns.reduce((sum, p) => sum + p.averageCases, 0) / 12;
			...
			// Death patterns
			const winterDeathAvg =
				winterMonths.reduce(
					(sum, month) => sum + patterns[month - 1].averageDeaths,
					0,
				) / 3;
			const overallDeathAvg =
				patterns.reduce((sum, p) => sum + p.averageDeaths, 0) / 12;

			if (winterDeathAvg > overallDeathAvg * 1.3 && overallDeathAvg > 0) {
				insights.push({
					pattern: "Winter Death Surge",
					description: "Higher death rates during winter months (Dec-Feb)",
					strength: Math.round((winterDeathAvg / overallDeathAvg - 1) * 100),
				});
			}

			const summerDeathAvg =
				summerMonths.reduce(
					(sum, month) => sum + patterns[month - 1].averageDeaths,
					0,
				) / 3;

			if (summerDeathAvg < overallDeathAvg * 0.7 && overallDeathAvg > 0) {
				insights.push({
					pattern: "Summer Death Low",
					description: "Lower death rates during summer months (Jun-Aug)",
					strength: Math.round((1 - summerDeathAvg / overallDeathAvg) * 100),
				});
			}
		...
		const analysis: SeasonalAnalysis = {
			...
			troughMonth,
			deathSeasonalityStrength:
				Math.round(deathSeasonalityStrength * 1000) / 1000,
			deathPeakMonth,
			deathTroughMonth,
			patterns,
		};
