...
import { APIProvider, Marker, InfoWindow } from '@vis.gl/react-google-maps';
import {
  useGeocodeLocation,
  useGetDirections,
  type Location,
...
const GoogleMapsContent = () => {
  ...
  useEffect(() => {
    if (geocodeQuery.data && geocodeQuery.data.length === 1) {
      setMapCenter({ lat: geocodeQuery.data[0].lat, lng: geocodeQuery.data[0].lng });
      setMapZoom(14);
    }
  }, [geocodeQuery.data]);
  ...
  const handleDirectionsFound = (directionsData: { origin: string; destination: string }) => {
      ...
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-stone-50 border border-stone-200 rounded-2xl shadow-sm p-8 max-w-md mx-auto text-center">
          ...
          <p className="text-stone-600 font-light mb-6">
            Google Maps API key is required to display the map. Please configure the
