...
export const fetchCountryData = async (countryCode: string): Promise<BaseData | undefined> => {
  const { data }: BaseResponse = await axios.get(`${BASE_URL}/countries/${countryCode}`);
...
export const fetchCountryRegionalData = async (
  ...
  return data
    ...
    .map(({ admin2: county, confirmed, deaths, lastUpdate, provinceState: state, uid }) => ({
      confirmed,
      county,
      deaths,
      lastUpdate,
