...
export const decodeSettings = (
	...
	try {
		...
		if (
			...
			parsedSettings.opacities.every(
				(o: unknown) => typeof o === "number" && o >= 0 && o <= 1,
			)
		) {
		...
		if (
			...
			parsedSettings.positions.every(
				(p: unknown) => typeof p === "number" && p >= 0 && p <= 100,
			)
		) {
		...
		if (
			parsedSettings?.glassGradient &&
			typeof parsedSettings.glassGradient === "object"
		) {
