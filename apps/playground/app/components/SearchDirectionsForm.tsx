...
interface SearchDirectionsFormProps {
  ...
  onDirectionsFound: (directions: { origin: string; destination: string }) => void;
  onClear: () => void;
  ...
  const handleCombinedForm = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isDirectionsMode) {
      ...
      if (origin.trim() && destination.trim()) {
        onDirectionsFound({ origin: origin.trim(), destination: destination.trim() });
      }
    } else {
      // Handle search mode
      const destination = formData.get('destination') as string;
              ...
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-red-600 text-xs mt-1">
                    {(isDirectionsMode ? directionsError : geocodeError)?.message ||
          ...
          <form onSubmit={handleCombinedForm} className="space-y-3">
            <div className="space-y-2">
              {stops.map((stop, index) => (
                <div key={`stop-${index}-${stop.slice(0, 10)}`} className="group relative">
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="flex-1">
                      ...
                      <input
                        type="text"
                        name={index === 0 && isDirectionsMode ? 'origin' : 'destination'}
                        id={`stop-${index}`}
                        value={stop}
      ...
      {directions && (
        <div className="absolute bottom-4 left-4 z-20">
          <div className="bg-white/95 backdrop-blur-md rounded-xl shadow-xl border border-white/20 p-4 w-80 max-h-64 overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold text-gray-800 font-serif">Route</h2>
            ...
            <div className="mb-3 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
              <div className="space-y-1 text-xs">
                <p className="font-medium text-blue-800">
                  From: <span className="font-normal">{directions.summary.from}</span>
                </p>
                <p className="font-medium text-blue-800">
                  To: <span className="font-normal">{directions.summary.to}</span>
                </p>
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center">
                    ...
                    <span className="text-blue-700 font-medium">{directions.summary.distance}</span>
                  </div>
                  <div className="flex items-center">
                    <svg
                      className="w-3 h-3 text-blue-600 mr-1"
                    ...
                    <span className="text-blue-700 font-medium">{directions.summary.duration}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              {directions.steps.map((step, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-xs text-gray-800">{step.instruction}</span>
                    {step.distance && (
                      <span className="block text-xs text-gray-500 mt-0.5">{step.distance}</span>
                    )}
                  </div>
                </li>
              ))}
