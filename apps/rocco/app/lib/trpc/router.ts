...
import { protectedProcedure, publicProcedure, router, userProcedure } from "./context";
...
const invitesRouter = router({
	getByList: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (!listItem) {
				throw new Error("List not found or you don't have permission to view invites");
			}
	...
	accept: protectedProcedure
		.input(z.object({ listId: z.string().uuid(), invitedUserEmail: z.string().email() }))
		.mutation(async ({ ctx, input }) => {
			if (!ctx.user) {
				throw new Error("User not found in context");
			}
			...
			if (!invite) {
				throw new Error("Invite not found or you don't have permission to accept it");
			}
